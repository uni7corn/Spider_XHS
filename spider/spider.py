...
class Data_Spider():
    ...
    def spider_some_note(self, notes: list, base_path: dict, save_choice: str, excel_name: str = '', proxies=None):
        """批量爬取并保存笔记。
        ...
        当 save_choice 为 ``all`` 或 ``excel`` 时，excel_name 不能为空。
        """
    ...
    def spider_user_all_note(self, user_url: str, base_path: dict, save_choice: str, excel_name: str = '', proxies=None):
        """获取一个用户的全部笔记，再按 save_choice 下载或写入 Excel。"""
        note_list = []
        try:
            ...
            self.spider_some_note(note_list, base_path, save_choice, excel_name, proxies)
        except Exception as e:
            success = False
            msg = e
    ...
    def spider_some_search_note(self, query: str, require_num: int, base_path: dict, save_choice: str, sort_type_choice=0, note_type=0, note_time=0, note_range=0, pos_distance=0, geo: dict = None, excel_name: str = '', proxies=None):
        """搜索指定数量的笔记并保存。

        参数说明：
          - sort_type_choice：0 综合，1 最新，2 最多点赞，3 最多评论，4 最多收藏。
          - note_type：0 不限，1 视频笔记，2 图文笔记。
          - note_time：0 不限，1 一天内，2 一周内，3 半年内。
          - note_range：0 不限，1 已看过，2 未看过，3 已关注。
          - pos_distance：0 不限，1 同城，2 附近；选择 1/2 时必须传 geo。
          - geo：``{"latitude": 39.9725, "longitude": 116.4207}``。
          - save_choice：见 spider_some_note()。
        """
        note_list = []
        try:
            success, msg, notes = self.xhs_apis.search_some_note(
                query, require_num, sort_type_choice, note_type, note_time, note_range, pos_distance, geo, proxies
            )
            if success:
...
if __name__ == '__main__':
    ...
    # 登录方式直接在代码里选择：
    #   cookie：使用用户登录后复制的完整 Cookie，或复用本项目之前登录保存的完整 Cookie。
    #   qrcode：本地生成二维码，用小红书 App 扫码；不打开浏览器。
    ...
    if login_type == 'qrcode':
        ...
        if not cookies_str:
            raise ValueError('cookie 登录模式需要在 .env 中填写 COOKIES')
        auth = XHSPcAuth.from_cookie(cookies_str)
    else:
        raise ValueError('login_type 只支持 cookie / qrcode / phone')
    ...
    data_spider.spider_some_note(notes, base_path, 'all', 'test')
    ...
    data_spider.spider_user_all_note(user_url, base_path, 'all')
    ...
    note_time = 0  # 0 不限, 1 一天内, 2 一周内, 3 半年内
